---
title: "Run_Crop2ML_SiriusQuality_SoilTemp"
author: "RCichota"
date: "`r Sys.Date()`"
...
A version of the SWAT soil temperature model from the Sirius Quality platform (https://eng-lepse.montpellier.hub.inrae.fr/modelling/process-based-model/siriusquality) was converted into R functions using Crop2ML (working version as of Oct/2024). The scripts below show how to use the model stand-alone in R. This is a simple demonstration on how to set up and run the model for a very simple example, as well as record and graph some results from this example. More elaborate exercises and coupling with other models are possible but not in scope here.
...
source(here("Crop2ML", "SoilTemperatureComponent.r"))
...
for (day in firstDay:lastDay)
{
 ...
 previousDeepLayerT <- onProcessState$deepLayerT
  ...
  simulatedTemperature <- c(simulatedTemperature, c(round((onProcessState$minTSoil=onProcessState$maxTSoil)/2, 6), rep(round(onProcessState$deepLayerT, 6),length(soilLayerData$Thickness))))
}
