...
if (testToRun == "One")
{
  temperatureModel <- modelsToRun[5]
  locationID <- locationIDsToRun[6]
  soilID <- soilsToRun[1]
  laiID <- laisToRun[3]
  pawID <- pawsToRun[3]
...
{
  # run all experiments (combinations of model, location, soil, LAI, and PAW)
  for (iMod in 1:length(modelsToRun))
  {
    temperatureModel <- modelsToRun[iMod]
    for (iLoc in 1:length(locationIDsToRun))
    {
      locationID <- locationIDsToRun[iLoc]
      for (iSoil in 1:length(soilsToRun))
      {
        soilID <- soilsToRun[iSoil]
        for (iLAI in 1:length(laisToRun))
        {
          laiID <- laisToRun[iLAI]
          for (iPAW in 1:length(pawsToRun))
          {
            pawID <- pawsToRun[iPAW]
            run_SensitivityScenario(temperatureModel, locationID, soilID, laiID, pawID)
          }
        }
      }
