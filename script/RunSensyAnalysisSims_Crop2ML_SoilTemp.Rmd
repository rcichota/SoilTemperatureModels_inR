...
temperatureModel <- modelsToRun[2]
testToRun <- "One"
...
soilWaterData <- soilWaterToRun
biomassData <- biomassToRun
...
run_PSCmodel <- function(siteData, soilGeneralData, soilLayerData, weatherData, soilWaterData, biomassData)
{
  ...
  initialState <- init_soiltemperatureswat(
    ...
    LayerThickness=soilLayerData$THICK/1000.0,
    LagCoefficient=0.8,
    AirTemperatureAnnualAverage=siteData$TAV,
    BulkDensity=soilLayerData$SLBDM,
    SoilProfileDepth=sum(soilLayerData$THICK)/1000.0)
  ...
  for (day in 1:nrow(weatherData))
  {
    onProcessState <- model_surfacepartonsoilswatc(
      ...
      AboveGroundBiomass=biomassData,
